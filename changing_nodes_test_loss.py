...
losshistory, train_state=model.train(callbacks=[time_callback], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a=list(map(np.sum, losshistory.loss_test))[0:30]
print("Total loss is {}".format(a))


print("This are the times {}".format(time_epochs))
print("This are the test losses {}".format(losshistory.loss_test[0:30]))
...
net_10 = dde.maps.FNN([2] + [50] * 6 + [1], "tanh", "Glorot uniform")
...
losshistory_1, train_state_1=model_1.train(callbacks=[time_callback_1], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a1=list(map(np.sum, losshistory_1.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
losshistory_2, train_state_2=model_2.train(callbacks=[time_callback_2], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a2=list(map(np.sum, losshistory_2.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
losshistory_3, train_state_3=model_3.train(callbacks=[time_callback_3], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a3=list(map(np.sum, losshistory_3.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
losshistory_4, train_state_4=model_4.train(callbacks=[time_callback_4], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a4=list(map(np.sum, losshistory_4.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
losshistory_5, train_state_5=model_5.train(callbacks=[time_callback_5], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a5=list(map(np.sum, losshistory_5.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
losshistory_6, train_state_6=model_6.train(callbacks=[time_callback_6], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
...
a6=list(map(np.sum, losshistory_6.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
model_7 = dde.Model(data, net_10)

model_7.compile("adam", lr=0.001)

class TimeHistory_7(dde.callbacks.Callback):
...
time_callback_7 = TimeHistory_7()
losshistory_7, train_state_7=model_7.train(callbacks=[time_callback_7], epochs=30000)
def fun(x):
    x=np.array(x)
    return np.sum(x)
chunks_17 = list(partition(1000, time_callback_7.times))
time_epochs_7=list(map(fun, chunks_17))
#model_7.compile("L-BFGS-B")

 #= model_7.train()

dde.saveplot(
    losshistory_7, train_state_7, issave=True, isplot=True, output_dir=OUTPUT_DIRECTORY
)
#print("Loss history gives {}".format(losshistory.loss_train))
...
a7=list(map(np.sum, losshistory_7.loss_test))[0:30]
#print("Total loss is {}".format(a))
...
time_taken7=[]
b=0
for i in range(len(time_epochs_7)):
    b=b+time_epochs_7[i]
    time_taken7.append(b)
...
plt.plot(time_taken7, a7, label="300 nodes")
